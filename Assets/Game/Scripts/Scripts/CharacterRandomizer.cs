...
namespace PsychoticLab
{
    ...
    public class CharacterRandomizer : MonoBehaviour
    {
        ...
        [HideInInspector] public List<GameObject> allAvaliablePartsList = new List<GameObject>(); // reference to camera transform, used for rotation around the model during or after a randomization (this is sourced from Camera.main, so the main camera must be in the scene for this to work)
        Transform camHolder;
        ...
        private void Start()
        {
            ...
            {
                allAvaliablePartsList.Add(tr.gameObject);
            }
            Debug.Log(allAvaliablePartsList.Count);
            // rebuild all lists
            BuildLists();
